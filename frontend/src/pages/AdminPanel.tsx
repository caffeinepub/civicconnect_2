...
import { useGetAllCustomers, useGetAllServiceRequests } from '../hooks/useQueries';
...
export default function AdminPanel() {
  ...
  const { data: customers, isLoading: customersLoading, error: customersError } = useGetAllCustomers();
  const { data: serviceRequests, isLoading: requestsLoading, error: requestsError } = useGetAllServiceRequests();
        ...
        <div className="w-full max-w-sm">
          ...
          >
            <div className="flex flex-col items-center mb-7">
              ...
              <p className="text-xs mt-1 text-center" style={{ color: 'oklch(0.60 0.03 255)' }}>
          ...
          >
            ...
            {customersLoading ? (
              <div className="flex items-center justify-center py-16">
                <Loader2 className="w-7 h-7 animate-spin" style={{ color: '#f5c518' }} />
              </div>
            ) : customersError ? (
              ...
              <p className="text-center py-16 text-sm" style={{ color: 'oklch(0.55 0.02 255)' }}>
                {customerSearch ? 'No customers match your search.' : 'No customers registered yet.'}
              </p>
            ) : (
              <>
                {/* Desktop Table */}
                  ...
                  <table className="w-full">
                    <thead>
                      <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                        {['ID', 'Full Name', 'Email', 'Mobile', 'Business Name', 'Registered'].map((h) => (
                          <th
                            key={h}
                    ...
                    <tbody>
                      {filteredCustomers.map((c, i) => (
                        ...
                        >
                          <td className="px-5 py-4 text-xs font-mono" style={{ color: 'oklch(0.60 0.03 255)' }}>
                            #{c.customerId.toString()}
                          </td>
                          <td className="px-5 py-4 text-sm font-medium text-white">{c.fullName}</td>
                          <td className="px-5 py-4 text-sm" style={{ color: 'oklch(0.70 0.03 255)' }}>
                            {c.email}
                          </td>
                          <td className="px-5 py-4 text-sm" style={{ color: 'oklch(0.70 0.03 255)' }}>
                            {c.mobileNumber}
                          ...
                          <td className="px-5 py-4 text-xs" style={{ color: 'oklch(0.60 0.03 255)' }}>
                ...
                <div className="md:hidden divide-y divide-white/10">
                  {filteredCustomers.map((c) => (
                    <div key={c.customerId.toString()} className="p-4 space-y-2">
                      ...
                      <p className="text-xs" style={{ color: 'oklch(0.65 0.03 255)' }}>{c.email}</p>
                      <p className="text-xs" style={{ color: 'oklch(0.65 0.03 255)' }}>{c.mobileNumber}</p>
                      <p className="text-xs" style={{ color: 'oklch(0.65 0.03 255)' }}>{c.businessName}</p>
        ...
        {activeTab === 'requests' && (
          ...
          >
            ...
            {requestsLoading ? (
              ...
              <p className="text-center py-16 text-sm" style={{ color: 'oklch(0.55 0.02 255)' }}>
                {requestSearch ? 'No requests match your search.' : 'No service requests yet.'}
              </p>
            ) : (
              <>
                {/* Desktop Table */}
                <div className="hidden md:block overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                        {['Req ID', 'Customer ID', 'Service Name', 'Notes', 'Submitted'].map((h) => (
                          <th
                            key={h}
                            className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider"
                            style={{ color: '#f5c518' }}
                    ...
                    <tbody>
                      {filteredRequests.map((r, i) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-4 text-sm font-medium text-white">{r.serviceName}</td>
                          <td className="px-5 py-4 text-sm" style={{ color: 'oklch(0.65 0.03 255)' }}>
                            {r.notes || '—'}
                          </td>
                          <td className="px-5 py-4 text-xs" style={{ color: 'oklch(0.60 0.03 255)' }}>
                ...
                <div className="md:hidden divide-y divide-white/10">
                  {filteredRequests.map((r) => (
                    <div key={r.requestId.toString()} className="p-4 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-bold text-white">{r.serviceName}</span>
                        <span className="text-xs font-mono" style={{ color: '#f5c518' }}>
                          #{r.requestId.toString()}
                        </span>
                      </div>
