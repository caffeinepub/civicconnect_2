...
export default function CustomerDashboard() {
  ...
  const handleUploadDocuments = () => {
    const subject = encodeURIComponent(`Document Upload - ${session.businessName}`);
        ...
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold text-white">
              Welcome back, <span style={{ color: '#f5c518' }}>{session.fullName}</span>!
            </h1>
            <p className="text-sm mt-1" style={{ color: 'oklch(0.65 0.03 255)' }}>
              <Building2 className="inline w-4 h-4 mr-1" style={{ color: '#f5c518' }} />
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h3 className="font-bold text-white text-base">{session.fullName}</h3>
              <p className="text-xs" style={{ color: 'oklch(0.60 0.03 255)' }}>Customer Account</p>
            </div>
          </div>

          <div className="space-y-3">
            <div className="flex items-start gap-2.5">
              <User className="w-4 h-4 mt-0.5 shrink-0" style={{ color: '#f5c518' }} />
              <div>
                <p className="text-xs" style={{ color: 'oklch(0.55 0.02 255)' }}>Full Name</p>
                <p className="text-sm text-white font-medium">{session.fullName}</p>
              </div>
            </div>
            <div className="flex items-start gap-2.5">
              <Mail className="w-4 h-4 mt-0.5 shrink-0" style={{ color: '#f5c518' }} />
              <div>
                <p className="text-xs" style={{ color: 'oklch(0.55 0.02 255)' }}>Email</p>
                <p className="text-sm text-white font-medium break-all">{session.email}</p>
              </div>
            </div>
            <div className="flex items-start gap-2.5">
              <Phone className="w-4 h-4 mt-0.5 shrink-0" style={{ color: '#f5c518' }} />
              <div>
                <p className="text-xs" style={{ color: 'oklch(0.55 0.02 255)' }}>Mobile</p>
                <p className="text-sm text-white font-medium">{session.mobileNumber}</p>
              </div>
            </div>
            <div className="flex items-start gap-2.5">
              <Building2 className="w-4 h-4 mt-0.5 shrink-0" style={{ color: '#f5c518' }} />
              <div>
                <p className="text-xs" style={{ color: 'oklch(0.55 0.02 255)' }}>Business Name</p>
                <p className="text-sm text-white font-medium">{session.businessName}</p>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          >
            ...
            <p className="text-sm mb-4" style={{ color: 'oklch(0.65 0.03 255)' }}>
              Select a service below and submit a request. Our team will contact you shortly.
            </p>
            ...
            <div className="relative mb-4">
              ...
              >
                <option value="" style={{ backgroundColor: '#0d1f3c', color: 'white' }}>
                  — Select a Service —
                </option>
                {SERVICES.map((svc) => (
                  <option key={svc} value={svc} style={{ backgroundColor: '#0d1f3c', color: 'white' }}>
                    {svc}
                  </option>
                ))}
              </select>
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-white">My Service Requests</h3>
            </div>

            {requestsLoading ? (
              <div className="flex items-center justify-center py-8">
                <Loader2 className="w-6 h-6 animate-spin" style={{ color: '#f5c518' }} />
              </div>
            ) : !serviceRequests || serviceRequests.length === 0 ? (
              <p className="text-sm text-center py-6" style={{ color: 'oklch(0.55 0.02 255)' }}>
              ...
              <div className="space-y-3">
                {serviceRequests.map((req) => (
                  ...
                  >
                    <div>
                      <p className="text-sm font-semibold text-white">{req.serviceName}</p>
                      <p className="text-xs mt-0.5" style={{ color: 'oklch(0.55 0.02 255)' }}>
                        Ref: #{req.requestId.toString()} · {formatTimestampShort(req.createdAt)}
                      </p>
                    </div>
                    <span
